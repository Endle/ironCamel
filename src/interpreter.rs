...
use crate::parser::{
    function2block, BlockAST, FunctionAST, LetBindingAST, ProgramAST, StatementAST,
};
...
use crate::builtin::{perform_write, IroncamelLinkedList};
    ...
    for s in &fun.statements {
        match &s {
            ...
            StatementAST::FileOpen(fo) => match fo.impure_procedure_name.as_str() {
                ...
                "fopen_write" => {
                    let mut fout =
                        std::fs::File::create(&fo.file_path).expect("Create file failed");
