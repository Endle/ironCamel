...
use crate::parser::{
    AST, BlockAST, DEBUG_TREE_INDENT, FunctionAST, LetBindingAST, ProgramAST, ReadAst,
    StatementAST, WriteAst,
};
use std::fmt;
...
impl AST for IfElseExpr {
    fn debug_strings(&self) -> Vec<String> {
        ...
        debug.push(format!(
            "if ({con})",
            con = build_expr_debug_strings(&self.condition).join(" ")
        ));
        debug.push(format!(
            "{ind}then {con}",
...
pub fn build_expr_debug_strings(expr: &ExprAST) -> Vec<String> {
    return match expr {
        ...
        ExprAST::Bool(b) => vec![format!(
            "Bool: {val}",
            val = if *b { "true" } else { "false" }
        )],
        ExprAST::Variable(v) => vec![format!("Variable: {val}", val = v)],
        ExprAST::StringLiteral(v) => vec![format!("Str: {val}", val = v)],
...
pub fn build_callable_object_debug_string(co: &CallableObject) -> String {
    match co {
        CallableObject::GlobalFunction(g) => {
            format!("Global function [{v}]", v = g)
        }
        CallableObject::BuiltinFunction(b) => {
            format!("Builtin function [{v}]", v = b)
        }
        CallableObject::Closure(clos, local_env) => {
            format!(
                "BindedClosure [{c}] in [{e}]",
