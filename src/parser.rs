...
use crate::expr::{try_read_expr, ExprAST};
use crate::tokenizer::Token;
use crate::tokenizer::Token::{
    IdentifierToken, KeywordFn, KeywordLet, LeftCurlyBracket, LeftParentheses, OperatorAssign,
    RightCurlyBracket, RightParentheses, Semicolon, SpaceToken,
};
...
fn read_function(tokens: &Vec<Token>, pos: usize) -> (FunctionAST, usize) {
    ...
    if let IdentifierToken(name) = &tokens[pos + len] {
        ...
        panic!(
            "Expect IdentfierToken for function name, got {:?}",
            tokens[pos + len]
        );
    }
...
pub fn read_argument_list(tokens: &Vec<Token>, pos: usize) -> (Vec<String>, usize) {
    ...
    'each_token: loop {
        debug!(
            "Try {:?} for read argument list, pos={}, len={}",
            tokens[pos + len],
            pos,
            len
        );
        if tokens[pos + len] == RightParentheses || tokens[pos + len] == Token::VerticalBar {
            break 'each_token;
        }
        match &tokens[pos + len] {
            ...
            _ => {
                panic!(
                    "Unexpected token when reading argument list {:?}",
                    tokens[pos + len]
                )
            }
        };
...
pub(crate) fn read_block(tokens: &Vec<Token>, pos: usize) -> (BlockAST, usize) {
    ...
    loop {
        ...
        assert!(sta_len > 0);
        debug!(
            "The statement consumed {} tokens: {:?}",
            sta_len,
            &tokens[pos + len..pos + len + sta_len]
        );
        len += sta_len;
    }
    ...
    match &return_val_len {
        None => panic!(
            "This block has no return expression! Got {:?}",
            tokens[pos + len]
        ),
        Some(_) => (),
    }
...
fn generate_stub_statement() -> (StatementAST, Option<usize>) {
    (StatementAST::Error, None)
}
...
fn try_read_io_operation(tokens: &Vec<Token>, pos: usize) -> (StatementAST, Option<usize>) {
    if tokens.len() - pos < 6 {
        return generate_stub_statement();
    }
    let procedure = match &tokens[pos] {
        IdentifierToken(s) => s,
        _ => {
            debug!("Not IO operation");
            return generate_stub_statement();
        }
    };
    if tokens[pos + 1] != Token::AddressSign {
        return generate_stub_statement();
    }
    let file_handler = match &tokens[pos + 2] {
        IdentifierToken(s) => s,
        _ => {
            panic!("Expect a file handle after @");
        }
    };
    match &tokens[pos + 3] {
        //read
        ...
        Token::OperatorPutTo => {
            ...
            let expr_len = match expr_len {
                Some(x) => x,
                None => {
                    panic!("Expect a valid expr when write!")
                }
            };
            len += expr_len;
        ...
        _ => {
            panic!("Expect << or >> or =, got {:?}", &tokens[pos + 3]);
        }
    }
}
...
fn generate_invalid_let_binding_ast() -> LetBindingAST {
    LetBindingAST {
        variable: INVALID_PLACEHOLDER.to_string(),
        expr: Box::new(ExprAST::Error),
    }
}
